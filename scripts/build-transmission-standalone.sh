...
rm -rf /tmp/transmission-src /tmp/transmission-build /work/out "$REPORT" /work/transmission-standalone.tar.gz
...
LDD_OUT=$(env -u LD_LIBRARY_PATH ldd "$DAEMON")
printf '%s\n' "$LDD_OUT"
if printf '%s\n' "$LDD_OUT" | grep -q "/opt/transmission-deps"; then
  echo "ERROR: staged daemon still resolves a library from the build-only prefix" >&2
  exit 1
fi
for private_lib in libcurl.so.4 libssl.so.3 libcrypto.so.3 libstdc++.so.6 libgcc_s.so.1; do
  printf '%s\n' "$LDD_OUT" | grep -F "$STAGE/lib/$private_lib"
done
...

{
  ...
  if [ "$ABI_FAIL" -eq 0 ]; then
    echo "GLIBC gate: PASS"
  else
    echo "GLIBC gate: FAIL"
  fi
} | tee -a "$REPORT"
...
ls -lh /work/transmission-standalone.tar.gz "$REPORT"
